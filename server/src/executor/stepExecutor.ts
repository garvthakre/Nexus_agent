...
/**
 * Safely resolve any path the AI might generate.
 * Handles Unix paths on Windows, ~ expansion, relative paths.
 */
function resolvePath(filePath: string): string {
  const home = os.homedir();

  // Fix Unix-style placeholder paths the AI hallucinates on Windows
  filePath = filePath
    .replace(/^~[\\/]/, home + path.sep)
    .replace(/^\/Users\/[^/\\]+[\\/]/, home + path.sep)
    .replace(/^\/home\/[^/\\]+[\\/]/, home + path.sep);

  // On Windows, normalize slashes
...
/**
 * Robust file downloader — follows redirects up to 10 hops.
 */
...
/**
 * Try multiple wallpaper image providers until one works.
...
async function fetchWallpaperImage(query: string, destPath: string): Promise<string> {
  ...
  const providers: Array<{ name: string; url: string }> = [
    // Picsum Photos — reliable, beautiful, always available
    { name: 'Picsum', url: `https://picsum.photos/seed/${enc}/1920/1080` },
    // LoremFlickr — topic-aware images
...
/**
 * set_wallpaper:
 * 1. Downloads a themed image from reliable providers (no AI-hallucinated URLs)
 * 2. Sets it as the desktop wallpaper using platform-native APIs
 */
async function setWallpaper(query: string | undefined): Promise<StepResult> {
  ...
  try {
    if (platform === 'win32') {
      // Write a temp PowerShell script to avoid shell quoting nightmares
...
`.trim();
      ...
      // Linux: try common DE methods in sequence
      const attempts = [
    ...
    return {
      success: true,
      message: `✓ Wallpaper set to "${query}" theme (source: ${provider})`,
      path: wallpaperPath,
    };
  } catch (err) {
    return {
      success: true,
      message: `Image downloaded to ${wallpaperPath} but auto-set failed`,
      path: wallpaperPath,
...
async function runShellCommand(command: string | undefined): Promise<StepResult> {
  ...
  try {
    const { stdout, stderr } = await execAsync(command, { timeout: 60_000, maxBuffer: 10 * 1024 * 1024 });
...
async function ensurePlaywright(): Promise<import('playwright').Page> {
  ...
  for (const opts of [{ ...base, channel: 'chrome' }, { ...base, channel: 'msedge' }, base] as object[]) {
    try { browserInstance = await chromium.launch(opts as Parameters<typeof chromium.launch>[0]); break; }
    catch (e) {
...
async function browserOpen(url: string | undefined): Promise<StepResult> {
  ...
  try {
    ...
    if (p === 'win32') await execAsync(`start "" "${url}"`, { timeout: 5000 });
    else if (p === 'darwin') await execAsync(`open "${url}"`, { timeout: 5000 });
    else await execAsync(`xdg-open "${url}"`, { timeout: 5000 });
    await sleep(1500);
...
async function browserFill(selector: string | undefined, value: string | undefined): Promise<StepResult> {
  ...
  const page = await ensurePlaywright();
  await sleep(500);
  for (const sel of [selector, `[name="${selector}"]`, `[id="${selector}"]`, `[placeholder*="${selector}" i]`, 'input:visible']) {
    try {
      await page.waitForSelector(sel, { timeout: 5000, state: 'visible' });
      await page.fill(sel, value);
      return { success: true, selector: sel, message: `Filled "${sel}"` };
    } catch { continue; }
  }
...
async function browserClick(selector: string | undefined): Promise<StepResult> {
  ...
  const page = await ensurePlaywright();
  await sleep(500);
  for (const sel of [selector, `[aria-label*="${selector}" i]`, `button:has-text("${selector}")`, `text="${selector}"`]) {
    try {
      await page.waitForSelector(sel, { timeout: 5000, state: 'visible' });
...
async function downloadFileCapability(url: string | undefined, destination: string | undefined): Promise<StepResult> {
  ...
  if (/example\.(com|org|net)|placeholder\.|your.?url|\/path\/to\/|localhost|127\.0\.0\.1/i.test(url)) {
    throw new Error(
      `"${url}" is a placeholder URL. ` +
      `For wallpapers use set_wallpaper instead. For downloads provide a real URL.`
    );
  }
